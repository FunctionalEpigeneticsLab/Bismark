...
on_targ_perc = read.delim('on_target.tsv')
rownames(on_targ_perc) = unlist(lapply(on_targ_perc$Sample, function(x){
    paste(unlist(strsplit(x, split = '_'))[1], unlist(strsplit(x, split = '_'))[2], sep = '_')
}))
both$On_target_percentage = on_targ_perc[rownames(both), 2]

on_targ_cov = read.delim('on_target.coverages', header = F)
rownames(on_targ_cov) = unlist(lapply(on_targ_cov$V1, function(x){
    paste(unlist(strsplit(x, split = '_'))[2], unlist(strsplit(x, split = '_'))[3], sep = '_')
}))
both$Target_regions_mean_coverage = on_targ_cov[rownames(both), 2]

on_targ_reads = read.delim('reads_on_target.tsv', header = F)
rownames(on_targ_reads) = unlist(lapply(on_targ_reads$V1, function(x){
    paste(unlist(strsplit(x, split = '_'))[2], unlist(strsplit(x, split = '_'))[3], sep = '_')
}))
both$On_target_reads = on_targ_reads[rownames(both), 2]
